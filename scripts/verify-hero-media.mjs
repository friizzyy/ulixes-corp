...
const optionalMasters = [
  ['design-assets/hero/ulixes-signal-desktop-master-4k.mp4', 3840, 2160],
  ['design-assets/hero/ulixes-signal-mobile-master-4k.mp4', 2160, 3840],
];
...
function verifyCadence(file, video) {
  const frameRate = ratio(video.avg_frame_rate);
  if (!Number.isFinite(frameRate) || frameRate < 29.9 || frameRate > 30.1) {
    fail(`${file} frame rate is ${frameRate}; expected approximately 30 fps`);
  }

  const frameCount = Number(video.nb_read_frames ?? video.nb_frames);
  if (!Number.isInteger(frameCount) || frameCount < 179 || frameCount > 181) {
    fail(`${file} contains ${frameCount} frames; expected approximately 180`);
  }
}
...
for (const [file, width, height] of optionalMasters) {
  if (!existsSync(file)) {
    continue;
  }
  ...
  if (video.codec_name !== 'hevc') {
    fail(`${file} codec is ${video.codec_name}; expected HEVC`);
  }
  if (video.width !== width || video.height !== height) {
    fail(`${file} is ${video.width}x${video.height}; expected ${width}x${height}`);
  }
