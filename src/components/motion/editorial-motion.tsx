...
export function Curtain({
  ...
  const shown = { y: 0 }
  /*
   * Mount-triggered by default, so hero type is already moving as the page
   * paints. `inView` defers the reveal for headlines further down, which
   * would otherwise finish long before the reader reaches them and leave the
   * section arriving flat.
   */
  const uncovered = reduced !== null && (!inView || seen)

  return (
    <Wrapper
      ref={ref}
      className={className ? `ed-curtain-mask ${className}` : 'ed-curtain-mask'}
      style={{ display: as === 'span' ? 'inline-block' : 'block' }}
    >
      <Inner
        initial={hidden}
        animate={uncovered ? shown : hidden}
...
export function Cascade({
  ...
  /*
   * `as` exists because several registers are numbered and are read as
   * ordered lists, and two of them are responsive sentinels the verification
   * harness selects on. Swapping their tag to satisfy the animation would
   * change both the semantics and the harness contract.
   *
   * Branched explicitly for the same reason the list stays a real list below:
   * an indexed motion[as] lookup resolved to undefined once bundled.
   */
  /* Registers carry composition and depth-plane attributes that both the CSS
     and the layout tests read off the list element itself, so they have to
     survive the swap to a motion list rather than move to a wrapper. */
  const shared = {
    ref,
    className,
    'aria-label': label,
  ...
  return as === 'ol' ? (
    <motion.ol {...shared}>{children}</motion.ol>
  ) : (
