...
export function Disclosure({ title, children, meta, defaultOpen = false, className }: DisclosureProps) {
  return (
    <details
      ...
      open={defaultOpen}
    >
      <summary className="flex cursor-pointer list-none items-start justify-between gap-4 min-h-[44px]">
        <div>
        ...
        <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-border text-text-muted transition-transform duration-200 group-open:rotate-45">
          +
        </span>
      </summary>
